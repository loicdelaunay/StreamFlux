...
class Streamlink extends EventEmitter {
    ...
    isLive(done) {
        module_childprocess.exec('streamlink -j ' + this.stream, (err, stdout, stderr) => {
            var json = JSON.parse(stdout);
            if (json.error);
            else {
                this.qualities = Object.keys(json['streams']);
                this.streamPlugin = json['plugin'];
                done(true);
            }
        });
    }
    ...
    start(done) {
        ...
        this.isLive(live => {
            ...
            this.live = module_childprocess.spawn('streamlink', args);
            this.live.stdout.on('data', (d) => {
                this.emit('log', d.toString());
            });
            this.live.stderr.on('data', (d) => {
                this.emit('log', d.toString());
            });
    ...
    getPlugin() {
        this.isLive(live => {
            this.emit('plugin', this.streamPlugin);
            return this;
        });
        this.emit('err', 'Stream is not live.');
    }
